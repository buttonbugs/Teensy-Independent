...
void get_accel_force_g(float& accel_x, float& accel_y, float& accel_z) {
    int16_t x, y, z;
    xl.readAxes(x, y, z);
    accel_x = xl.convertToG(ACCEL_MAX_SCALE, x) + ACCEL_X_OFFSET;
    accel_y = xl.convertToG(ACCEL_MAX_SCALE, y) + ACCEL_Y_OFFSET;
    accel_z = xl.convertToG(ACCEL_MAX_SCALE, z) + ACCEL_Z_OFFSET;
}
